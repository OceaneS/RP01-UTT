...
cat(paste("La valeur actuelle avec le cas 2 sans revente est de", VA(14, c(rep(1.422, times=35),53), tauxInflation), "% du prix de la voiture.\n"))
mensualite <- uniroot(function(m) VA(14, c(rep(m, times=35),53/1.422*m), tauxInflation) - valeurCible, c(0,100), extendInt="yes")$root
cat(paste("Il faudrait que le 2eme cas soit plus interessant que les mensualites soit inferieures a", mensualite, "%.\n"))
...
cat(paste("La valeur actuelle avec le cas 2 avec revente est de", VA(14, rep(1.422, times=35), tauxInflation[1:35]), "% du prix de la voiture.\n"))
